...
function createInsertText(
  functionName: string,
  parameters: CompletionDocItem["parameters"],
  type: "function" | "transform"
): string {
  if (type === "transform") {
    ...
    const paramPlaceholders = additionalParams
      .map((param, index) => {
        const placeholder = param.optional
          ? `\${${index + 1}:${param.name}?}`
          : `\${${index + 1}:${param.name}}`;
        return placeholder;
      })
...
function mapTypeToCompletionType(
  grammarType: string
): "function" | "transform" {
...
for (const [type, lib] of Object.entries(libs)) {
  for (const key in lib) {
    ...
    const doc = typedoc.children.find(
    ...
    const description = (signature as any).comment?.summary?.[0]?.text || "";
    const examples = extractExamples(
      (signature as any).comment?.summary || [],
      (signature as any).comment?.blockTags || []
    );
    ...
    if (completionType === "function") {
      ...
      if (!seenItems.has(functionKey)) {
        ...
        const existingItem = completionDocs.find(
      ...
      if (parameters.length > 0) {
        ...
        if (!seenItems.has(transformKey)) {
          ...
          const existingItem = completionDocs.find(
            (item) => item.name === functionName && item.type === "transform"
          );
          if (existingItem && key !== functionName) {
            if (!existingItem.aliases) {
              existingItem.aliases = [];
